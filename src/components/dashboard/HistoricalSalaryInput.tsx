...
interface HistoricalSalaryInputProps {
  simulationInput: SimulationInput;
  salaries: SalaryPeriod[];
  onSalariesChange: (salaries: SalaryPeriod[]) => void;
}

export function HistoricalSalaryInput({ 
  simulationInput, 
  salaries, 
  onSalariesChange 
}: HistoricalSalaryInputProps) {
    ...
    <div className="space-y-4">
      {salaries.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          <p>No historical salary adjustments added yet.</p>
          <p className="text-sm mt-2">Click "Add Period" to specify exact salaries for past years.</p>
        </div>
      ) : (
          ...
          {salaries.map((salary) => (
            <div key={salary.id} className="flex gap-4 items-end p-4 border border-border rounded-lg">
              <div className="flex-1">
                <Label htmlFor={`year-${salary.id}`}>Year</Label>
                <Input
                  id={`year-${salary.id}`}
                  type="number"
              ...
              <div className="flex-1">
                <Label htmlFor={`amount-${salary.id}`}>Monthly Salary (PLN)</Label>
                <Input
                  id={`amount-${salary.id}`}
                  type="number"
                  min={0}
              ...
              <Button
                ...
                aria-label="Remove salary period"
              >
